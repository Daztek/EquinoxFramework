...
void AddPatchOperation(json jPatchArray, string sOp, string sPath, json jValue)
{
    json jOperation = JsonObject();
    JsonObjectSetInplace(jOperation, "op", JsonString(sOp));
    JsonObjectSetInplace(jOperation, "path", JsonString(sPath));
    JsonObjectSetInplace(jOperation, "value", jValue);
    JsonArrayInsertInplace(jPatchArray, jOperation);
}

json GffGetFieldObject(string sType, json jValue)
{
    json jField = JsonObject();
    JsonObjectSetInplace(jField, "type", JsonString(sType));
    JsonObjectSetInplace(jField, "value", jValue);
    return jField;
}

json GffAddField(json jGff, string sLabel, string sType, json jValue, int nType = -1)
{
    json jField = GffGetFieldObject(sType, jValue);
    if (sType == GFF_FIELD_TYPE_STRUCT && nType != -1)
        JsonObjectSetInplace(jField, "__struct_id", JsonInt(nType));
    json jPatch = JsonArray();
    AddPatchOperation(jPatch, "add", "/" + sLabel, jField);
...
{
    json jPatch = JsonArray();
    AddPatchOperation(jPatch, "test", "/" + sLabel + "/type", JsonString(sType));
    AddPatchOperation(jPatch, "replace", "/" + sLabel + "/value", jValue);
    return JsonPatch(jGff, jPatch);
}
...
json GffRemoveField(json jGff, string sLabel, string sType)
{
    json jPatch = JsonArray();
    AddPatchOperation(jPatch, "test", "/" + sLabel + "/type", JsonString(sType));
    AddPatchOperation(jPatch, "remove", "/" + sLabel, JsonNull());
    return JsonPatch(jGff, jPatch);
}
...
json GffLocString(string v, int nStrRef = -1)
{
    ...
    if (v != "")
        JsonObjectSetInplace(jLocString, "0", JsonString(v)); // english/any
    if (nStrRef != -1)
        JsonObjectSetInplace(jLocString, "id", JsonInt(nStrRef));
...
json GffCreateObject(int nObjectType)
{
    ...
    json ret = JsonObject();
    JsonObjectSetInplace(ret, "__data_type", JsonString(ot));
    return ret;
}
...
json GffCreateArea(json jARE, json jGIT)
{
    ...
    JsonObjectSetInplace(jCAF, "__data_type", JsonString("CAF "));
...
int GffGetFieldExists(json jGff, string sLabel, string sType = "")
{
    ...
    return sType == "" ? JsonGetType(jFieldType) : jFieldType == JsonString(sType);
}
