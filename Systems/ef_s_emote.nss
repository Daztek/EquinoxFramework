...
{
    NB_InitializeWindow(NuiRect(-1.0f, -1.0f, 200.0f, 300.0f));
...
void Emote_ToggleWindow()
{
    ...
    if (NWM_GetIsWindowOpen(oPlayer, EMOTE_WINDOW_ID))
        ...
        NWM_SetBind("buttons", GetLocalJsonOrDefault(GetDataObject(EMOTE_SCRIPT_NAME), EMOTE_NAME_ARRAY, JsonArray()));
}
...
{
    object oPlayer = OBJECT_SELF;
    int nEmote = JsonArrayGetInt(GetLocalJsonOrDefault(GetDataObject(EMOTE_SCRIPT_NAME), EMOTE_ID_ARRAY, JsonArray()), NuiGetEventArrayIndex());
...
void Emote_AddEmote(string sEmoteName, int nEmoteId)
{
    ...
    SetLocalJson(oDataObject, EMOTE_NAME_ARRAY, JsonArrayInsertString(GetLocalJsonOrDefault(oDataObject, EMOTE_NAME_ARRAY, JsonArray()), sEmoteName));
    SetLocalJson(oDataObject, EMOTE_ID_ARRAY, JsonArrayInsertInt(GetLocalJsonOrDefault(oDataObject, EMOTE_ID_ARRAY, JsonArray()), nEmoteId));
}
